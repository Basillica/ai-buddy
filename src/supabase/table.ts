import { PostgrestError, RealtimeChannel, UserMetadata } from "@supabase/supabase-js";
import { APIHandler } from "./base";
import { ChatRoomModel, MessageModel, UserModel } from "../models";
import { Setter } from "solid-js";

export class TableAPIHandler extends APIHandler {
    ...
    async insertRows(bookings: MessageModel[] | ChatRoomModel[] | any): Promise<any[] | null> {
    ...
    async subAllToEvents(setNewMessage: Setter<{ [key: string]: any }>): Promise<void> {
    ...
    async subToInserts(setNewMessage: Setter<{ [key: string]: any }>): Promise<void> {
        const channels = this.getClient()
    ...
    async subToUpdates(setNewMessage: Setter<{ [key: string]: any }>): Promise<void> {
        const channels = this.getClient()
    ...
    async subToDeletes(setNewMessage: Setter<{ [key: string]: any }>): Promise<void> {
        const channels = this.getClient()
    ...
    async subToRow(column: string, value: any, setNewMessage: Setter<{ [key: string]: any }>): Promise<void> {
        const channels = this.getClient()
